...
// echo($id_user_from);
     $ikos = mysqli_query($connect, "SELECT `groupp` FROM `user` WHERE `id_user`='$id_user_from' ");
     $ikos = mysqli_fetch_all($ikos);
     foreach ($ikos as $ikos) {
        $needed_gr=$ikos[0];
    // echo($ikos[0]);
    }
...
<a href="../../inc/singup/logout.php">ВЫЙТИ</a>
  ...
  <!-- <form action="group_people.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="group" value="<?=$group?>">
            <button type="submit"> Назад</button>
            </form> -->
  <a class="button-inlec-back"  href="uprav.php?id=<?=$needed_gr?>">Назад</a>
  <a class="button-inlec-back-dlin" href="../people/sbros.php?id=<?=$users[0]?>">Сбросить попытку теста</a>
  </div>
